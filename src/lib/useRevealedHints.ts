...
import { orderHints } from "./hintOrder"
...
// Which properties the player has been shown outright. This lives above the hint panel because the
// knowledge panel needs it too: a hint is evidence about the answer exactly as a guess is, and showing
// it in one place while the deduction panel claims to hold everything known would be a lie.
export function useRevealedHints({
  ...
  // guess rows leave it out of their own ordering.
  ...
  return {
    revealedEntries,
    allRevealed: revealTarget >= hintable.length,
    ...
    hintsLeft: Math.max(0, Math.min(maxHints - hintsUsed, hintable.length - revealTarget)),
  }
}
